...
describe('VTG query references', () => {
  ...
  it.each([
  ...
  ] as const)('decodes the $mode replacement for 5-6', async ({ isAnti, p0, p1 }) => {
    const version = await loadSpiroAnimQSVersion(1)
    const queryCodec = await useSpiroAnimQS(version.VDEF, useBaseQS(version.VDEF), 1)
    ...
    const selection = { reference: '5-6', speedRatio: '1:1', isAnti } as const
    const pattern = getVtgPatternDefinition(selection)?.patternsBySpeedRatio['1:1']?.(selection)
  ...
  it.each([
  ...
  ] as const)('decodes the $mode replacement for 6-6', async ({ isAnti, p0, p1 }) => {
    const version = await loadSpiroAnimQSVersion(1)
    const queryCodec = await useSpiroAnimQS(version.VDEF, useBaseQS(version.VDEF), 1)
    const readable = encodeReadable(
      queryCodec.decodeQS({
    ...
    const selection = { reference: '6-6', speedRatio: '1:1', isAnti } as const
    const pattern = getVtgPatternDefinition(selection)?.patternsBySpeedRatio['1:1']?.(selection)
